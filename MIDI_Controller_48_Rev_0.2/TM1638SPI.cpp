...
TM1638SPI::TM1638SPI(byte strobePin, SPISettings* _spiSetting, boolean activateDisplay, byte intensity)
	: TM16XX(strobePin, _spiSetting, 8, activateDisplay, intensity)
{
	// nothing else to do - calling super is enough
}

void TM1638SPI::setDisplayToHexNumber(unsigned long number, byte dots, boolean leadingZeros,
...
void TM1638SPI::setDisplayToDecNumberAt(unsigned long number, byte dots, byte startingPos, boolean leadingZeros,
                                        const byte numberFont[])
{
  if (number > 99999999L) {
    setDisplayToError();
...
void TM1638SPI::setDisplayToDecNumber(unsigned long number, byte dots, boolean leadingZeros,
                                      const byte numberFont[])
{
	setDisplayToDecNumberAt(number, dots, 0, leadingZeros, numberFont);
}

void TM1638SPI::setDisplayToSignedDecNumber(signed long number, byte dots, boolean leadingZeros,
                                            const byte numberFont[])
{
	if (number >= 0) {
		setDisplayToDecNumberAt(number, dots, 0, leadingZeros, numberFont);
...
void TM1638SPI::setDisplayToBinNumber(byte number, byte dots, const byte numberFont[])
{
  for (int i = 0; i < displays; i++) {
...
void TM1638SPI::setLED(byte color, byte pos)
{
    sendData((pos << 1) + 1, color);
}

void TM1638SPI::setLEDs(word leds)
{
  for (int i = 0; i < displays; i++) {
    byte color = 0;
...
byte TM1638SPI::getButtons(void)
{
  byte keys = 0;
...
void TM1638SPI::sendChar(byte pos, byte data, boolean dot)
{
